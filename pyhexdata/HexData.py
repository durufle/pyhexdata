...
class HexData:
    ...
    def to_send_cmd_string(self, extended_LC=False):
        ...
        if len(self) > 0:
            ...
            # CLA
            apdu_format += "CLA"
            cmd +=         " {:02x}".format(self._data[index])
            index += 1
            ...
            # P1 P2
            apdu_format += " | P1 P2"
            cmd +=         " | {0:02x} {1:02x}".format(self._data[index], self._data[index+1])
            index += 2
            ...
            if ln > index:
                ...
                # DATA
                apdu_format += " | " + " "*(3*(ln-index) - 3) + "DATA"
                cmd += " |   "
                cmd +=         " ".join(["{:02x}".format(self._data[x]) for x in range(index, ln)])

            apdu_format += " | "
            cmd += " | "

            return apdu_format, cmd

        return None

    def to_cmd_resp_string(self):
        """
        This function converts the numpy array to string with spaces
        ...
        if len(self) > 0:
            ...
            if ln > 2:
                # DATA
                apdu_format += " "*(3*(ln-2) - 3) + "DATA | "
                cmd += "  " + " ".join(["{:02x}".format(self._data[x]) for x in range(0, ln-2)]) + " | "
            elif ln == 2:
                pass
            else:
                return apdu_format, cmd
            ...
            apdu_format += "SW1 | SW2"
            cmd += " {0:02x} |  {1:02x}".format(self._data[-2], self._data[-1])
